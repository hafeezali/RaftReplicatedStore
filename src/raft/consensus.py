...
class Consensus(raftdb_grpc.ConsensusServicer) :
    ...
    def create_log_entry_request(self, prev_log_index, entry):
        ...
        term = self.__log.get_term()
        self.logger.debug("Fetched current term")
    ...
    def AppendEntries(self, request, context):
        ...
        # random bs above 
            ...
            self.logger.debug(f'Previous entry matches in the log for key - {request.key} and value - {request.value}')
            ...
            self.logger.debug(f'Previous entry does not match in the log for key - {request.key} and value - {request.value}')
            return raftdb.LogEntryResponse(code=500, term = self.__log.get_term())
